...
uses messages, windows, classes, controls, stdctrls,sysutils, graphics, buttons,
     ...
     QRPrntr, Quickrpt, QR3Const, qrexpr;
...
{$ifndef ver110}
  case Alignment of
    ...
    TaRightJustify: begin
        SetTextAlign(aCanvas.Handle, TA_Right + TA_Top + TA_NoUpdateCP);
        X := X + aWidth;
        if fsItalic in Font.Style then X := X - (ParentReport.TextWidth(Font, ' '));
      end;
    TaCenter : begin
        SetTextAlign(aCanvas.Handle, TA_Center + TA_Top + TA_NoUpdateCP);
        X := X + aWidth / 2;
...
begin
  ...
  // Set the room for what is available
  if Stretch then // Always fill complete area no need to check for zoom !!!
    Dest := ClientRect
  else
    begin
      Pw := trunc(Picture.Width  * (Zoom / 100.00));
      Ph := trunc(Picture.Height * (Zoom / 100.00)) ;
      if Center then
        Dest := Bounds((Width - Pw) div 2, (Height - PH) div 2,
          PW, PH)
      else
        Dest := Rect(0, 0, PW, PH);
    end;
...
//      Result := SendMessage(ARichEdit.Handle, EM_DISPLAYBAND, 0, Longint(@Range.rc))
    else

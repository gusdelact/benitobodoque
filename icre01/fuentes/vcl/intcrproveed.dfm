object WCrProveedor: TWCrProveedor
  Left = 280
  Top = 146
  Width = 627
  Height = 486
  Caption = 'Proveedor / Acreditado PYME'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = True
  OnClose = FormClose
  OnDestroy = FormDestroy
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object lbID_ACREDITADO: TLabel
    Left = 8
    Top = 35
    Width = 49
    Height = 13
    Caption = 'Proveedor'
  end
  object lbID_PROV_EXTERNO: TLabel
    Left = 8
    Top = 61
    Width = 97
    Height = 13
    Caption = 'Identificador Externo'
  end
  object lbF_ALTA: TLabel
    Left = 16
    Top = 346
    Width = 66
    Height = 13
    Caption = 'Fecha de Alta'
  end
  object lbF_MODIFICA: TLabel
    Left = 16
    Top = 369
    Width = 73
    Height = 13
    Caption = 'Fecha Modifica'
  end
  object lbCVE_USU_MODIFICA: TLabel
    Left = 304
    Top = 370
    Width = 79
    Height = 13
    Caption = 'Usuario Modifica'
  end
  object LBCVE_USU_ALTA: TLabel
    Left = 304
    Top = 345
    Width = 57
    Height = 13
    Caption = 'Usuario Alta'
  end
  object bvDatosPYME: TBevel
    Left = 8
    Top = 169
    Width = 585
    Height = 5
    Shape = bsTopLine
    Style = bsRaised
  end
  object lbDatosPYME: TLabel
    Left = 16
    Top = 154
    Width = 136
    Height = 13
    Caption = ' Datos de Acreditado PYME '
  end
  object lbID_PROM_ADM_PYME: TLabel
    Left = 8
    Top = 180
    Width = 81
    Height = 13
    Caption = 'Promotor Admon.'
  end
  object lbIMP_VENTA_ANUAL: TLabel
    Left = 8
    Top = 206
    Width = 78
    Height = 13
    Caption = 'Imp Venta Anual'
  end
  object lbNUM_PER_OCUPADO: TLabel
    Left = 8
    Top = 231
    Width = 88
    Height = 13
    Caption = 'Personal Ocupado'
  end
  object lbPCT_PROD_MER_EXT: TLabel
    Left = 304
    Top = 231
    Width = 117
    Height = 13
    Caption = '% Prod Mercado Externo'
  end
  object lbPCT_PROD_MER_INT: TLabel
    Left = 304
    Top = 206
    Width = 114
    Height = 13
    Caption = '% Prod Mercado Interno'
  end
  object lbCVE_SECTOR_PRO: TLabel
    Left = 8
    Top = 91
    Width = 101
    Height = 13
    Caption = 'Sector por Proveedor'
  end
  object Label1: TLabel
    Left = 8
    Top = 113
    Width = 83
    Height = 13
    Caption = 'Nombre Contacto'
  end
  object Label8: TLabel
    Left = 8
    Top = 10
    Width = 106
    Height = 13
    Caption = 'Entidad Descontadora'
  end
  object Label2: TLabel
    Left = 16
    Top = 253
    Width = 84
    Height = 13
    Caption = 'Saldos Proveedor'
    OnClick = btACREDITADOClick
  end
  object Bevel1: TBevel
    Left = 8
    Top = 268
    Width = 585
    Height = 5
    Shape = bsTopLine
    Style = bsRaised
  end
  object lbIMP_RIESGO_MAX: TLabel
    Left = 8
    Top = 281
    Width = 44
    Height = 13
    Caption = 'Asignado'
  end
  object Label3: TLabel
    Left = 251
    Top = 281
    Width = 47
    Height = 13
    Caption = 'Dispuesto'
  end
  object Label4: TLabel
    Left = 424
    Top = 281
    Width = 49
    Height = 13
    Caption = 'Disponible'
  end
  object edID_ACREDITADO: TEdit
    Tag = 562
    Left = 141
    Top = 31
    Width = 73
    Height = 21
    HelpContext = 1100
    CharCase = ecUpperCase
    MaxLength = 8
    TabOrder = 3
    OnExit = edID_ACREDITADOExit
  end
  object edID_PROV_EXTERNO: TEdit
    Tag = 18
    Left = 141
    Top = 57
    Width = 100
    Height = 21
    HelpContext = 1200
    CharCase = ecUpperCase
    MaxLength = 10
    TabOrder = 6
    OnExit = edID_PROV_EXTERNOExit
  end
  object rgSIT_PROVEEDOR: TRadioGroup
    Tag = 18
    Left = 8
    Top = 303
    Width = 585
    Height = 35
    HelpContext = 2100
    Caption = 'Situación Proveedor'
    Columns = 2
    ItemIndex = 0
    Items.Strings = (
      'Activo'
      'Inactivo')
    TabOrder = 20
    OnExit = rgSIT_PROVEEDORExit
  end
  object btACREDITADO: TBitBtn
    Tag = 50
    Left = 220
    Top = 31
    Width = 22
    Height = 20
    HelpContext = 1101
    TabOrder = 4
    OnClick = btACREDITADOClick
    OnExit = edID_ACREDITADOExit
    Glyph.Data = {
      
      0303030303030303030303030303030303030303030303030303}
    NumGlyphs = 2
  end
  object MsgPanel: TPanel
    Left = 1
    Top = 390
    Width = 595
    Height = 25
    TabOrder = 25
  end
  object PnDatos: TPanel
    Left = 1
    Top = 415
    Width = 595
    Height = 21
    TabOrder = 26
    object lbEmpresa: TLabel
      Left = 11
      Top = 2
      Width = 42
      Height = 8
      Caption = 'EMPRESA :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbUsuario: TLabel
      Left = 11
      Top = 9
      Width = 44
      Height = 8
      Caption = 'USUARIO : '
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbFecha: TLabel
      Left = 248
      Top = 2
      Width = 50
      Height = 8
      Caption = 'FECHA HOY :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbPerfil: TLabel
      Left = 248
      Top = 9
      Width = 33
      Height = 8
      Caption = 'PERFIL :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDUsuario: TLabel
      Left = 75
      Top = 9
      Width = 6
      Height = 8
      Caption = ' |'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDPerfil: TLabel
      Left = 312
      Top = 9
      Width = 6
      Height = 8
      Caption = ' |'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDFecha: TLabel
      Left = 312
      Top = 2
      Width = 6
      Height = 8
      Caption = ' |'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDEmpresa: TLabel
      Left = 75
      Top = 2
      Width = 6
      Height = 8
      Caption = ' |'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object edF_ALTA: TEdit
    Left = 141
    Top = 341
    Width = 103
    Height = 21
    TabStop = False
    Color = clBtnFace
    TabOrder = 21
  end
  object edF_MODIFICA: TEdit
    Left = 141
    Top = 366
    Width = 103
    Height = 21
    TabStop = False
    Color = clBtnFace
    TabOrder = 23
  end
  object edCVE_USU_ALTA: TEdit
    Left = 485
    Top = 341
    Width = 106
    Height = 21
    TabStop = False
    Color = clBtnFace
    TabOrder = 22
  end
  object edCVE_USU_MODIFICA: TEdit
    Left = 485
    Top = 366
    Width = 106
    Height = 21
    TabStop = False
    Color = clBtnFace
    TabOrder = 24
  end
  object edNOMBRE_ACREDITADO: TEdit
    Left = 248
    Top = 31
    Width = 348
    Height = 21
    TabStop = False
    Color = clBtnFace
    TabOrder = 5
  end
  object chELIMINAR: TCheckBox
    Tag = 18
    Left = 528
    Top = 81
    Width = 62
    Height = 21
    HelpContext = 1400
    Alignment = taLeftJustify
    Caption = 'Eliminar'
    TabOrder = 10
    OnClick = chELIMINARClick
    OnExit = chELIMINARExit
  end
  object edID_PROM_ADM_PYME: TEdit
    Tag = 530
    Left = 141
    Top = 176
    Width = 76
    Height = 21
    HelpContext = 1600
    CharCase = ecUpperCase
    TabOrder = 12
    OnExit = btPROM_ADMExit
  end
  object edNUM_PER_OCUPADO: TEdit
    Tag = 18
    Left = 141
    Top = 227
    Width = 100
    Height = 21
    HelpContext = 1800
    CharCase = ecUpperCase
    TabOrder = 16
    Text = '0'
    OnExit = edNUM_PER_OCUPADOExit
  end
  object edNOMBRE_PROM_ADM: TEdit
    Left = 248
    Top = 176
    Width = 348
    Height = 21
    TabStop = False
    Color = clBtnFace
    ReadOnly = True
    TabOrder = 14
  end
  object btPROM_ADM: TBitBtn
    Tag = 18
    Left = 220
    Top = 176
    Width = 22
    Height = 21
    HelpContext = 1601
    TabOrder = 13
    OnClick = btPROM_ADMClick
    OnExit = btPROM_ADMExit
    Glyph.Data = {
      
      0303030303030303030303030303030303030303030303030303}
    NumGlyphs = 2
  end
  object edIMP_VENTA_ANUAL: TInterEdit
    Tag = 18
    Left = 141
    Top = 202
    Width = 100
    Height = 21
    HelpContext = 1700
    Prefijo = '1'
    Contrato = -1
    TipoReader = trDinero
    OnExit = edIMP_VENTA_ANUALExit
    MaxLength = 12
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBtnText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    TabOrder = 15
    UseReType = True
    UseSep = True
    UseDisplay = True
    PositiveOnly = False
    ColorPos = clBtnText
    ColorNeg = clRed
  end
  object edPCT_PROD_MER_EXT: TInterEdit
    Tag = 18
    Left = 497
    Top = 227
    Width = 98
    Height = 21
    HelpContext = 2000
    Prefijo = '1'
    Contrato = -1
    TipoReader = trDinero
    OnExit = edPCT_PROD_MER_EXTExit
    MaxLength = 12
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBtnText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    TabOrder = 18
    UseReType = False
    UseSep = True
    UseDisplay = True
    PositiveOnly = False
    ColorPos = clBtnText
    ColorNeg = clRed
  end
  object edPCT_PROD_MER_INT: TInterEdit
    Tag = 18
    Left = 497
    Top = 202
    Width = 98
    Height = 21
    HelpContext = 1900
    Prefijo = '1'
    Contrato = -1
    TipoReader = trDinero
    OnExit = edPCT_PROD_MER_INTExit
    MaxLength = 12
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBtnText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    TabOrder = 17
    UseReType = False
    UseSep = True
    UseDisplay = True
    PositiveOnly = False
    ColorPos = clBtnText
    ColorNeg = clRed
  end
  object edCVE_SECTOR_PRO: TEdit
    Tag = 530
    Left = 141
    Top = 83
    Width = 76
    Height = 21
    HelpContext = 1300
    CharCase = ecUpperCase
    TabOrder = 7
    OnExit = btCVE_SECTOR_PRExit
  end
  object btCVE_SECTOR_PR: TBitBtn
    Tag = 18
    Left = 220
    Top = 83
    Width = 22
    Height = 21
    HelpContext = 1301
    TabOrder = 8
    OnClick = btCVE_SECTOR_PRClick
    OnExit = btCVE_SECTOR_PRExit
    Glyph.Data = {
      
      0303030303030303030303030303030303030303030303030303}
    NumGlyphs = 2
  end
  object edDESC_SECTOR: TEdit
    Left = 246
    Top = 83
    Width = 275
    Height = 21
    TabStop = False
    Color = clBtnFace
    ReadOnly = True
    TabOrder = 9
  end
  object edNOM_CONTACTO: TEdit
    Tag = 18
    Left = 141
    Top = 109
    Width = 452
    Height = 21
    HelpContext = 1500
    CharCase = ecUpperCase
    MaxLength = 80
    TabOrder = 11
    OnExit = edNOM_CONTACTOExit
  end
  object edCVE_FND_ENT_DES: TEdit
    Tag = 562
    Left = 141
    Top = 6
    Width = 73
    Height = 21
    HelpContext = 1000
    CharCase = ecUpperCase
    MaxLength = 8
    TabOrder = 0
    OnExit = edCVE_FND_ENT_DESExit
  end
  object btnCVE_FND_ENT_DES: TBitBtn
    Tag = 50
    Left = 220
    Top = 6
    Width = 22
    Height = 20
    HelpContext = 1001
    TabOrder = 1
    OnClick = btnCVE_FND_ENT_DESClick
    OnExit = btnCVE_FND_ENT_DESExit
    Glyph.Data = {
      
      0303030303030303030303030303030303030303030303030303}
    NumGlyphs = 2
  end
  object edDESC_ENT_DES: TEdit
    Left = 248
    Top = 6
    Width = 348
    Height = 21
    TabStop = False
    Color = clBtnFace
    TabOrder = 2
  end
  object edIMP_DISPUESTO: TInterEdit
    Left = 308
    Top = 277
    Width = 97
    Height = 21
    TabStop = False
    Prefijo = '1'
    Contrato = -1
    TipoReader = trDinero
    DisplayMask = '###,###,###,##0.00'
    MaxLength = 16
    Color = clBtnFace
    TabOrder = 27
    UseReType = True
    UseSep = True
    UseDisplay = True
    PositiveOnly = False
    ColorPos = clBtnText
    ColorNeg = clRed
  end
  object ieIMP_LIM_ASIGNADO: TInterEdit
    Tag = 18
    Left = 140
    Top = 277
    Width = 97
    Height = 21
    HelpContext = 2050
    Prefijo = '1'
    Contrato = -1
    TipoReader = trDinero
    OnExit = ieIMP_LIM_ASIGNADOExit
    DisplayMask = '###,###,###,##0.00'
    MaxLength = 16
    TabOrder = 19
    UseReType = True
    UseSep = True
    UseDisplay = True
    PositiveOnly = False
    ColorPos = clBtnText
    ColorNeg = clRed
  end
  object edIMP_DISPONIBLE: TInterEdit
    Left = 495
    Top = 277
    Width = 97
    Height = 21
    TabStop = False
    Prefijo = '1'
    Contrato = -1
    TipoReader = trDinero
    DisplayMask = '###,###,###,##0.00'
    MaxLength = 16
    Color = clBtnFace
    TabOrder = 28
    UseReType = True
    UseSep = True
    UseDisplay = True
    PositiveOnly = False
    ColorPos = clBtnText
    ColorNeg = clRed
  end
  object bbFactoElec: TBitBtn
    Tag = 17
    Left = 8
    Top = 134
    Width = 585
    Height = 18
    Caption = 'Tiene Factoraje Electrónico'
    TabOrder = 29
    TabStop = False
    OnClick = bbFactoElecClick
  end
  object InterForma1: TInterForma
    IsMain = True
    OnDespuesNuevo = InterForma1DespuesNuevo
    OnDespuesModificar = InterForma1DespuesModificar
    OnDespuesCancelar = InterForma1DespuesCancelar
    OnDespuesAceptar = InterForma1DespuesAceptar
    OnAntesAceptar = InterForma1AntesAceptar
    OnBuscar = InterForma1Buscar
    msgSinRegistros = 'No existe un Registro para realizar la operación.'
    msgModificarRegistro = 'Imposible Modificar el Registro Actual.'
    msgEliminarRegistro = 'Imposible Eliminar el Registro Actual.'
    msgCrearRegistro = 'Imposible Crear el Registro Actual.'
    msgAceptarRegistro = 'Imposible Aceptar los cambios del Registro Actual.'
    msgCancelarRegistro = 'Imposible Cancelar los cambios del Registro Actual.'
    msgPreguntaEliminar = '¿Desea Eliminar el Registro Actual?'
    FormaEstilo = feHorizontal
    CanEdit = False
    IsNewData = False
    Version = 20
    HideButtons = False
    ShowBtnPreview = False
    ShowBtnImprimir = False
    ShowNavigator = False
    ShowError = False
    IsCancel = False
    Left = 546
    Top = 37
  end
  object ilACREDITADO: TInterLinkit
    Control = edID_ACREDITADO
    OnEjecuta = ilACREDITADOEjecuta
    TipoReader = trTexto
    CharCase = ecUpperCase
    OnCapture = ilACREDITADOCapture
    Left = 256
    Top = 35
  end
  object ilPROMADM: TInterLinkit
    Control = edID_PROM_ADM_PYME
    OnEjecuta = ilPROMADMEjecuta
    TipoReader = trTexto
    CharCase = ecUpperCase
    Left = 275
    Top = 185
  end
  object ilSECTOR: TInterLinkit
    Control = edCVE_SECTOR_PRO
    OnEjecuta = ilSECTOREjecuta
    TipoReader = trTexto
    CharCase = ecUpperCase
    Left = 267
    Top = 85
  end
  object ilCVE_FND_ENT_DES: TInterLinkit
    Control = edCVE_FND_ENT_DES
    OnEjecuta = ilCVE_FND_ENT_DESEjecuta
    TipoReader = trTexto
    CharCase = ecUpperCase
    OnCapture = ilCVE_FND_ENT_DESCapture
    Left = 256
    Top = 3
  end
end

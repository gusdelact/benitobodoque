object WMEdoCtaC: TWMEdoCtaC
  Left = 310
  Top = 118
  Width = 568
  Height = 493
  Caption = 'Estado de Cuenta Certificado'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = True
  OnClose = FormClose
  OnDestroy = FormDestroy
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object GroupBox1: TGroupBox
    Left = 394
    Top = 3
    Width = 159
    Height = 49
    Caption = 'Folio de Solicitud de Impresión'
    TabOrder = 0
    object edFOLIO_SOLICITUD: TEdit
      Left = 30
      Top = 18
      Width = 105
      Height = 21
      TabStop = False
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 0
    end
  end
  object PnDatos: TPanel
    Left = 0
    Top = 431
    Width = 553
    Height = 21
    TabOrder = 4
    object lbEmpresa: TLabel
      Left = 11
      Top = 2
      Width = 42
      Height = 8
      Caption = 'EMPRESA :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbUsuario: TLabel
      Left = 11
      Top = 9
      Width = 44
      Height = 8
      Caption = 'USUARIO : '
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbFecha: TLabel
      Left = 248
      Top = 2
      Width = 50
      Height = 8
      Caption = 'FECHA HOY :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbPerfil: TLabel
      Left = 248
      Top = 9
      Width = 33
      Height = 8
      Caption = 'PERFIL :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDUsuario: TLabel
      Left = 75
      Top = 9
      Width = 6
      Height = 8
      Caption = ' |'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDPerfil: TLabel
      Left = 312
      Top = 9
      Width = 6
      Height = 8
      Caption = ' |'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDFecha: TLabel
      Left = 312
      Top = 2
      Width = 6
      Height = 8
      Caption = ' |'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDEmpresa: TLabel
      Left = 75
      Top = 2
      Width = 6
      Height = 8
      Caption = ' |'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object GroupBox2: TGroupBox
    Left = 0
    Top = 158
    Width = 553
    Height = 253
    TabOrder = 2
    object Label3: TLabel
      Left = 10
      Top = 21
      Width = 140
      Height = 13
      Caption = 'Fecha de Emisión Edo de Cta'
    end
    object Label4: TLabel
      Left = 347
      Top = 21
      Width = 85
      Height = 13
      Caption = 'F. Firma Convenio'
    end
    object Label5: TLabel
      Left = 10
      Top = 40
      Width = 60
      Height = 13
      Caption = 'Encabezado'
    end
    object Label6: TLabel
      Left = 10
      Top = 128
      Width = 27
      Height = 13
      Caption = 'Calce'
    end
    object Label7: TLabel
      Left = 10
      Top = 219
      Width = 77
      Height = 13
      Caption = 'Grupo de Firmas'
    end
    object edF_FIRMA_CONV: TEdit
      Tag = 18
      Left = 440
      Top = 17
      Width = 80
      Height = 21
      MaxLength = 10
      TabOrder = 2
      OnExit = edF_FIRMA_CONVExit
    end
    object dtpF_FIRMA_CONV: TInterDateTimePicker
      Tag = 18
      Left = 520
      Top = 17
      Width = 21
      Height = 21
      CalAlignment = dtaLeft
      Date = 38574.4597612153
      Time = 38574.4597612153
      DateFormat = dfShort
      DateMode = dmComboBox
      Kind = dtkDate
      ParseInput = False
      TabOrder = 3
      TabStop = False
      Edit = edF_FIRMA_CONV
      Formato = dfDate
    end
    object edF_EMIS_EDO_CTA: TEdit
      Tag = 18
      Left = 169
      Top = 17
      Width = 80
      Height = 21
      MaxLength = 10
      TabOrder = 0
      OnExit = edF_EMIS_EDO_CTAExit
    end
    object dtpF_EMIS_EDO_CTA: TInterDateTimePicker
      Tag = 18
      Left = 249
      Top = 17
      Width = 21
      Height = 21
      CalAlignment = dtaLeft
      Date = 38574.4597612153
      Time = 38574.4597612153
      DateFormat = dfShort
      DateMode = dmComboBox
      Kind = dtkDate
      ParseInput = False
      TabOrder = 1
      TabStop = False
      Edit = edF_EMIS_EDO_CTA
      Formato = dfDate
    end
    object edTX_ENCABEZADO: TMemo
      Tag = 18
      Left = 102
      Top = 40
      Width = 438
      Height = 86
      MaxLength = 500
      ScrollBars = ssVertical
      TabOrder = 4
      OnExit = edTX_ENCABEZADOExit
    end
    object edTX_CALCE: TMemo
      Tag = 18
      Left = 103
      Top = 128
      Width = 438
      Height = 86
      MaxLength = 500
      ScrollBars = ssVertical
      TabOrder = 5
      OnExit = edTX_CALCEExit
    end
    object edNOM_FIRMA: TEdit
      Left = 208
      Top = 215
      Width = 334
      Height = 21
      TabStop = False
      Color = clBtnFace
      TabOrder = 8
    end
    object btCVE_FIRMA: TBitBtn
      Tag = 18
      Left = 184
      Top = 215
      Width = 21
      Height = 21
      TabOrder = 7
      OnClick = btCVE_FIRMAClick
      OnExit = edCVE_FIRMAExit
      Glyph.Data = {
        
        0303030303030303030303030303030303030303030303030303}
      NumGlyphs = 2
    end
    object edCVE_FIRMA: TEdit
      Tag = 530
      Left = 104
      Top = 215
      Width = 80
      Height = 21
      CharCase = ecUpperCase
      MaxLength = 6
      TabOrder = 6
      OnExit = edCVE_FIRMAExit
    end
  end
  object GroupBox3: TGroupBox
    Left = 0
    Top = 52
    Width = 553
    Height = 106
    TabOrder = 1
    object lbID_ACREDITADO: TLabel
      Left = 3
      Top = 21
      Width = 51
      Height = 13
      Caption = 'Acreditado'
    end
    object Label1: TLabel
      Left = 3
      Top = 43
      Width = 54
      Height = 13
      Caption = 'Disposición'
    end
    object Label2: TLabel
      Left = 363
      Top = 73
      Width = 73
      Height = 13
      Caption = 'Fecha de Corte'
    end
    object edID_ACREDITADO: TEdit
      Tag = 530
      Left = 102
      Top = 17
      Width = 80
      Height = 21
      CharCase = ecUpperCase
      MaxLength = 10
      TabOrder = 0
      OnExit = edID_ACREDITADOExit
    end
    object btID_ACREDITADO: TBitBtn
      Tag = 18
      Left = 182
      Top = 17
      Width = 21
      Height = 21
      TabOrder = 1
      OnClick = btID_ACREDITADOClick
      OnExit = edID_ACREDITADOExit
      Glyph.Data = {
        
        0303030303030303030303030303030303030303030303030303}
      NumGlyphs = 2
    end
    object edNOM_ACREDITADO: TEdit
      Left = 207
      Top = 17
      Width = 334
      Height = 21
      TabStop = False
      Color = clBtnFace
      TabOrder = 2
    end
    object edDESC_CREDITO: TEdit
      Left = 207
      Top = 39
      Width = 334
      Height = 21
      TabStop = False
      Color = clBtnFace
      TabOrder = 5
    end
    object btID_CREDITO: TBitBtn
      Tag = 18
      Left = 182
      Top = 39
      Width = 21
      Height = 21
      TabOrder = 4
      OnClick = btID_CREDITOClick
      OnExit = edID_CREDITOExit
      Glyph.Data = {
        
        0303030303030303030303030303030303030303030303030303}
      NumGlyphs = 2
    end
    object edID_CREDITO: TEdit
      Tag = 530
      Left = 102
      Top = 39
      Width = 80
      Height = 21
      CharCase = ecUpperCase
      MaxLength = 10
      TabOrder = 3
      OnExit = edID_CREDITOExit
    end
    object rgSITUACION: TRadioGroup
      Tag = 18
      Left = 5
      Top = 63
      Width = 345
      Height = 32
      Caption = 'Situación'
      Columns = 4
      ItemIndex = 0
      Items.Strings = (
        'Vigente'
        'Impagado'
        'Vencido'
        'Todos')
      TabOrder = 6
      OnExit = rgSITUACIONExit
    end
    object dtpF_ALTA: TInterDateTimePicker
      Tag = 18
      Left = 520
      Top = 69
      Width = 21
      Height = 21
      CalAlignment = dtaLeft
      Date = 38574.4597612153
      Time = 38574.4597612153
      DateFormat = dfShort
      DateMode = dmComboBox
      Kind = dtkDate
      ParseInput = False
      TabOrder = 8
      TabStop = False
      Edit = edF_ALTA
      Formato = dfDate
    end
    object edF_ALTA: TEdit
      Tag = 18
      Left = 440
      Top = 69
      Width = 80
      Height = 21
      MaxLength = 10
      TabOrder = 7
      OnExit = edF_ALTAExit
    end
  end
  object MsgPanel: TPanel
    Left = 0
    Top = 410
    Width = 553
    Height = 21
    TabOrder = 3
  end
  object ilID_CREDITO: TInterLinkit
    Control = edID_CREDITO
    OnEjecuta = ilID_CREDITOEjecuta
    TipoReader = trTexto
    CharCase = ecUpperCase
    OnCapture = ilID_CREDITOCapture
    Left = 216
    Top = 90
  end
  object InterForma1: TInterForma
    IsMain = True
    OnDespuesNuevo = InterForma1DespuesNuevo
    OnDespuesModificar = InterForma1DespuesModificar
    OnAntesAceptar = InterForma1AntesAceptar
    OnBuscar = InterForma1Buscar
    msgSinRegistros = 'No existe un Registro para realizar la operación.'
    msgModificarRegistro = 'Imposible Modificar el Registro Actual.'
    msgEliminarRegistro = 'Imposible Eliminar el Registro Actual.'
    msgCrearRegistro = 'Imposible Crear el Registro Actual.'
    msgAceptarRegistro = 'Imposible Aceptar los cambios del Registro Actual.'
    msgCancelarRegistro = 'Imposible Cancelar los cambios del Registro Actual.'
    msgPreguntaEliminar = '¿Desea Eliminar el Registro Actual?'
    CanEdit = False
    IsNewData = False
    Version = 20
    HideButtons = False
    ShowBtnPreview = False
    ShowBtnImprimir = False
    ShowNavigator = False
    ShowError = False
    IsCancel = False
    Left = 314
    Top = 20
  end
  object ilID_ACREDITADO: TInterLinkit
    Control = edID_ACREDITADO
    OnEjecuta = ilID_ACREDITADOEjecuta
    TipoReader = trTexto
    CharCase = ecUpperCase
    OnCapture = ilID_ACREDITADOCapture
    Left = 216
    Top = 60
  end
  object ilCVE_FIRMA: TInterLinkit
    Control = edCVE_FIRMA
    OnEjecuta = ilCVE_FIRMAEjecuta
    TipoReader = trTexto
    CharCase = ecUpperCase
    OnCapture = ilCVE_FIRMACapture
    Left = 216
    Top = 370
  end
end

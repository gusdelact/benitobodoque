// Sistema         : Clase de CR_GA_DOC_COB
// Fecha de Inicio : 06/07/2004
// Función forma   : Clase de CR_GA_DOC_COB
...
constructor TMDocCob.Create( AOwner : TComponent );
begin Inherited;
   ...
   SIT_DOC_COB :=CreaCampo('SIT_DOC_COB',ftString,tsNinguno,tsNinguno,True);
   With SIT_DOC_COB Do Begin
      Size:=2;
      UseCombo:=True;
      ComboLista.Add('0'); ComboDatos.Add('AC');
      ComboLista.Add('1'); ComboDatos.Add('IN');
   end;
      SIT_DOC_COB.Caption:='Situación';
      SIT_DOC_COB.NoCompare:= True;
   ...
   TableName := 'CR_GA_DOC_COB';
   UseQuery := True;
   HelpFile := 'IntMDocCob.Hlp';
...
Destructor TMDocCob.Destroy;
begin
   Inherited;
end;

function TMDocCob.ShowWindow(FormaTipo:TFormaTipo):Integer;
var W : TWMDocCob;
begin
   W:=TWMDocCob.Create(Self);
   try
      W.Objeto:= Self;
...
Function TMDocCob.InternalBusca:Boolean;
Var T : TInterFindit;
begin
   InternalBusca := False;
   T := CreaBuscador('IMDocCob.it','I');
   Try
...
function TMDocCob.Reporte:Boolean;
begin
   Result := False;
end;
...
(***********************************************FORMA CR_GA_DOC_COB********************)
(*                                                                              *)
(*  FORMA DE CR_GA_DOC_COB                                                            *)
(*                                                                              *)
(***********************************************FORMA CR_GA_DOC_COB********************)

procedure TwMDocCob.FormShow(Sender: TObject);
begin
   ...
   With Objeto Do Begin
      ...
      SIT_DOC_COB.Control        := rgSITUACION;
...
procedure TwMDocCob.FormDestroy(Sender: TObject);
begin
   With Objeto Do Begin
      ID_SOLICITUD.Control        := Nil;
      IMP_GARANTIA.Control        := Nil;
      SIT_DOC_COB.Control         := Nil;
      F_ALTA.Control              := Nil;
...
procedure TwMDocCob.FormClose(Sender: TObject; var Action: TCloseAction);
begin Action := caFree;
end;

procedure TwMDocCob.InterForma1Buscar(Sender: TObject);
Var
   sSQL: String;
   qyQuery: TQuery;
begin
...
procedure TwMDocCob.InterForma1DespuesNuevo(Sender: TObject);
begin
...
procedure TwMDocCob.rgSITUACIONExit(Sender: TObject);
begin
   InterForma1.ValidaExit(rgSITUACION,True,CNULL,True);
end;

procedure TwMDocCob.InterForma1DespuesModificar(Sender: TObject);
begin
...
procedure TwMDocCob.InterForma1AntesAceptar(Sender: TObject;
...
procedure TwMDocCob.edID_SOLICITUDExit(Sender: TObject);
...
Begin
   If ((Objeto.Modo = moEdit) Or (Objeto.Modo=moAppEnd)) Then Begin
      ...
      End Else Begin
         sSQL:= 'SELECT * FROM CR_GA_DOC_COB WHERE ID_SOLICITUD='+Objeto.ID_SOLICITUD.AsSQL;
...
procedure TwMDocCob.ilID_SOLICITUDCapture(Sender: TObject;
  var Show: Boolean);
begin
   Show:=(InterForma1.CanEdit)and (InterForma1.IsNewData);
end;

procedure TwMDocCob.ilID_SOLICITUDEjecuta(Sender: TObject);
begin
...
procedure TwMDocCob.btID_SOLICITUDClick(Sender: TObject);
begin
...
procedure TwMDocCob.edIMP_GARANTIAExit(Sender: TObject);
Var  vlSalida  :  boolean;
     vlMsg     :  String;
Begin
   If ((Objeto.Modo = moEdit) Or (Objeto.Modo=moAppEnd)) Then Begin
...
procedure TwMDocCob.InterForma1DespuesEliminar(Sender: TObject);
begin
   edCVE_GARANTIA.Text:='';
   edNOM_ID_SOLICITUD.Text:='';
end;

procedure TwMDocCob.InterForma1DespuesShow(Sender: TObject);
begin
   If Objeto.MRegistro.ID_SOLICITUD.AsInteger <> 0 Then Begin

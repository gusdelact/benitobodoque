object wMParApor: TwMParApor
  Left = 221
  Top = 150
  Width = 563
  Height = 262
  Caption = 'Catálogo de Garantías de Participaciones/Aportaciones Federales'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = True
  OnClose = FormClose
  OnDestroy = FormDestroy
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label6: TLabel
    Left = 0
    Top = 5
    Width = 77
    Height = 13
    Caption = 'No. de Garantía'
  end
  object Label3: TLabel
    Left = 0
    Top = 26
    Width = 95
    Height = 13
    Caption = 'Nombre de Cedente'
  end
  object Label1: TLabel
    Left = 0
    Top = 47
    Width = 28
    Height = 13
    Caption = 'Ramo'
  end
  object Label2: TLabel
    Left = 0
    Top = 68
    Width = 103
    Height = 13
    Caption = 'No. de Cuenta Banco'
  end
  object Label4: TLabel
    Left = 190
    Top = 68
    Width = 131
    Height = 13
    Caption = 'No. Contrato Casa de Bolsa'
  end
  object PnlMsg: TPanel
    Left = 0
    Top = 193
    Width = 552
    Height = 21
    TabOrder = 13
  end
  object PnDatos: TPanel
    Left = 0
    Top = 214
    Width = 552
    Height = 21
    TabOrder = 14
    object lbEmpresa: TLabel
      Left = 11
      Top = 2
      Width = 42
      Height = 8
      Caption = 'EMPRESA :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbUsuario: TLabel
      Left = 11
      Top = 9
      Width = 44
      Height = 8
      Caption = 'USUARIO : '
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbFecha: TLabel
      Left = 262
      Top = 2
      Width = 50
      Height = 8
      Caption = 'FECHA HOY :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbPerfil: TLabel
      Left = 262
      Top = 9
      Width = 33
      Height = 8
      Caption = 'PERFIL :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDUsuario: TLabel
      Left = 75
      Top = 9
      Width = 6
      Height = 8
      Caption = ' |'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDPerfil: TLabel
      Left = 326
      Top = 9
      Width = 6
      Height = 8
      Caption = ' |'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDFecha: TLabel
      Left = 326
      Top = 2
      Width = 6
      Height = 8
      Caption = ' |'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDEmpresa: TLabel
      Left = 75
      Top = 2
      Width = 6
      Height = 8
      Caption = ' |'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -7
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object GroupBox4: TGroupBox
    Left = 0
    Top = 136
    Width = 552
    Height = 58
    Caption = 'Registro'
    TabOrder = 12
    object lbF_ALTA: TLabel
      Left = 25
      Top = 37
      Width = 51
      Height = 13
      Caption = 'Fecha Alta'
    end
    object lbF_MODIF: TLabel
      Left = 296
      Top = 37
      Width = 73
      Height = 13
      Caption = 'Fecha Modifica'
    end
    object lbUSUA_ALTA: TLabel
      Left = 25
      Top = 16
      Width = 57
      Height = 13
      Caption = 'Usuario Alta'
    end
    object lbUSUA_MODIF: TLabel
      Left = 296
      Top = 16
      Width = 79
      Height = 13
      Caption = 'Usuario Modifica'
    end
    object edF_ALTA: TEdit
      Left = 104
      Top = 33
      Width = 110
      Height = 21
      TabStop = False
      CharCase = ecUpperCase
      Color = clBtnFace
      ReadOnly = True
      TabOrder = 1
    end
    object edF_MODIFICA: TEdit
      Left = 433
      Top = 33
      Width = 110
      Height = 21
      TabStop = False
      CharCase = ecUpperCase
      Color = clBtnFace
      ReadOnly = True
      TabOrder = 3
    end
    object edCVE_USU_ALTA: TEdit
      Left = 104
      Top = 12
      Width = 110
      Height = 21
      TabStop = False
      CharCase = ecUpperCase
      Color = clBtnFace
      MaxLength = 8
      ReadOnly = True
      TabOrder = 0
    end
    object edCVE_USU_MODIF: TEdit
      Left = 433
      Top = 12
      Width = 110
      Height = 21
      TabStop = False
      CharCase = ecUpperCase
      Color = clBtnFace
      MaxLength = 8
      ReadOnly = True
      TabOrder = 2
    end
  end
  object rgSITUACION: TRadioGroup
    Tag = 18
    Left = 0
    Top = 103
    Width = 552
    Height = 33
    Hint = 'Situación de garantías personales'
    HelpContext = 1600
    Caption = 'Situación'
    Columns = 2
    ItemIndex = 0
    Items.Strings = (
      'Activa'
      'Inactiva')
    ParentShowHint = False
    ShowHint = True
    TabOrder = 11
    OnExit = rgSITUACIONExit
  end
  object edID_SOLICITUD: TInterEdit
    Tag = 512
    Left = 106
    Top = 1
    Width = 80
    Height = 21
    Hint = 'Identificador único del registro de garantía'
    HelpContext = 1000
    TabStop = False
    Prefijo = '1'
    Contrato = -1
    TipoReader = trEntero
    OnExit = edID_SOLICITUDExit
    MaxLength = 10
    Color = clBtnFace
    TabOrder = 0
    CharCase = ecUpperCase
    UseReType = False
    UseSep = True
    UseDisplay = False
    PositiveOnly = True
    ColorPos = clBtnText
    ColorNeg = clRed
  end
  object btID_SOLICITUD: TBitBtn
    Tag = 50
    Left = 190
    Top = 1
    Width = 21
    Height = 21
    HelpContext = 1001
    Enabled = False
    TabOrder = 1
    TabStop = False
    OnClick = btID_SOLICITUDClick
    OnExit = edID_SOLICITUDExit
    Glyph.Data = {
      
      0303030303030303030303030303030303030303030303030303}
    NumGlyphs = 2
  end
  object edNOM_ID_SOLICITUD: TEdit
    Left = 285
    Top = 1
    Width = 267
    Height = 21
    Hint = 'Descripción del tipo de garantía'
    TabStop = False
    Color = clBtnFace
    ReadOnly = True
    TabOrder = 3
  end
  object edCVE_GARANTIA: TEdit
    Left = 218
    Top = 1
    Width = 65
    Height = 21
    Hint = 'Clave de la garantía'
    TabStop = False
    CharCase = ecUpperCase
    Color = clBtnFace
    MaxLength = 6
    ParentShowHint = False
    ReadOnly = True
    ShowHint = True
    TabOrder = 2
  end
  object edNOM_CEDENTE: TEdit
    Tag = 18
    Left = 106
    Top = 22
    Width = 445
    Height = 21
    Hint = 'Nombre de Cedente'
    HelpContext = 1100
    CharCase = ecUpperCase
    MaxLength = 60
    TabOrder = 4
    OnExit = edNOM_CEDENTEExit
  end
  object edCVE_TIPO_RAMO: TEdit
    Tag = 530
    Left = 106
    Top = 43
    Width = 80
    Height = 21
    Hint = 'Clave de Tipo de Ramo'
    HelpContext = 1200
    CharCase = ecUpperCase
    MaxLength = 6
    TabOrder = 5
    OnExit = edCVE_TIPO_RAMOExit
  end
  object btCVE_TIPO_RAMO: TBitBtn
    Tag = 18
    Left = 190
    Top = 43
    Width = 21
    Height = 21
    HelpContext = 1201
    TabOrder = 6
    OnClick = btCVE_TIPO_RAMOClick
    OnExit = edCVE_TIPO_RAMOExit
    Glyph.Data = {
      
      0303030303030303030303030303030303030303030303030303}
    NumGlyphs = 2
  end
  object edDESC_RAMO: TEdit
    Left = 218
    Top = 43
    Width = 334
    Height = 21
    Hint = 'Descripción de tipo de Ramo'
    TabStop = False
    CharCase = ecUpperCase
    Color = clBtnFace
    MaxLength = 60
    ReadOnly = True
    TabOrder = 7
  end
  object edCUENTA_BANCO: TEdit
    Tag = 18
    Left = 106
    Top = 64
    Width = 80
    Height = 21
    HelpContext = 1300
    CharCase = ecUpperCase
    MaxLength = 20
    TabOrder = 8
    OnExit = edCUENTA_BANCOExit
  end
  object edID_CONTRATO: TInterEdit
    Tag = 18
    Left = 338
    Top = 64
    Width = 80
    Height = 21
    HelpContext = 1400
    Prefijo = '1'
    Contrato = -1
    TipoReader = trContrato
    OnExit = edID_CONTRATOExit
    MaxLength = 10
    TabOrder = 9
    CharCase = ecUpperCase
    UseReType = False
    UseSep = True
    UseDisplay = False
    PositiveOnly = True
    ColorPos = clBtnText
    ColorNeg = clRed
  end
  object cbxB_FIDE_MAESTRO: TCheckBox
    Tag = 18
    Left = 107
    Top = 88
    Width = 123
    Height = 17
    HelpContext = 1500
    Caption = 'Fideicomiso Maestro'
    TabOrder = 10
    OnExit = cbxB_FIDE_MAESTROExit
  end
  object InterForma1: TInterForma
    IsMain = True
    OnDespuesNuevo = InterForma1DespuesNuevo
    OnDespuesModificar = InterForma1DespuesModificar
    OnDespuesEliminar = InterForma1DespuesEliminar
    OnAntesAceptar = InterForma1AntesAceptar
    OnBuscar = InterForma1Buscar
    msgSinRegistros = 'No existe un Registro para realizar la operación.'
    msgModificarRegistro = 'Imposible Modificar el Registro Actual.'
    msgEliminarRegistro = 'Imposible Eliminar el Registro Actual.'
    msgCrearRegistro = 'Imposible Crear el Registro Actual.'
    msgAceptarRegistro = 'Imposible Aceptar los cambios del Registro Actual.'
    msgCancelarRegistro = 'Imposible Cancelar los cambios del Registro Actual.'
    msgPreguntaEliminar = '¿Desea Eliminar el Registro Actual?'
    FormaEstilo = feVertical
    CanEdit = False
    IsNewData = False
    Version = 20
    HideButtons = False
    ShowBtnPreview = False
    ShowBtnImprimir = False
    ShowNavigator = False
    ShowError = False
    IsCancel = False
    OnDespuesShow = InterForma1DespuesShow
    Left = 453
    Top = 65530
  end
  object ilID_SOLICITUD: TInterLinkit
    Control = edID_SOLICITUD
    OnEjecuta = ilID_SOLICITUDEjecuta
    TipoReader = trEntero
    CharCase = ecNormal
    OnCapture = ilID_SOLICITUDCapture
    Left = 247
    Top = 65533
  end
  object ilCVE_TIPO_RAMO: TInterLinkit
    Control = edCVE_TIPO_RAMO
    OnEjecuta = ilCVE_TIPO_RAMOEjecuta
    TipoReader = trTexto
    CharCase = ecUpperCase
    OnCapture = ilCVE_TIPO_RAMOCapture
    Left = 223
    Top = 41
  end
end

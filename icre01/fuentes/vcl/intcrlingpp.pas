...
IntParamCre,
IntCRBCLiGPP //Bitácora de Cambios Líneas Garantía Primeras Pérdidas
;

Type
 TCrLinGPP= class;

  TWCrLinGPPerd=Class(TForm)
...
constructor TCrLinGPP.Create( AOwner : TComponent ); 
begin Inherited; 
      ...
      StpName  := ' '; 
      UseQuery := False; 
      HelpFile := 'InTCrLinGPP.Hlp';
      ShowMenuReporte:=True;
end;

Destructor TCrLinGPP.Destroy;
begin inherited;
end;


function TCrLinGPP.ShowWindow(FormaTipo:TFormaTipo):Integer;
var W : TWCrLinGPPerd;
begin
   W:=TWCrLinGPPerd.Create(Self);
   try
      W.Objeto:= Self;
...
{
Function TCrLinGPP.InternalBusca:Boolean;
Var T:TInterFindit;
...
{
function TCrLinGPP.Reporte:Boolean;
...
procedure TWCrLinGPPerd.FormShow(Sender: TObject);
begin
...
procedure TWCrLinGPPerd.FormDestroy(Sender: TObject);
begin
...
procedure TWCrLinGPPerd.FormClose(Sender: TObject; var Action: TCloseAction);
begin Action := caFree;
end;

procedure TWCrLinGPPerd.InterForma1DespuesNuevo(Sender: TObject);
begin //<<Control>>.SetFocus;
end;

procedure TWCrLinGPPerd.InterForma1DespuesModificar(Sender: TObject);
begin //<<Control>>.SetFocus;
end;
...
procedure TWCrLinGPPerd.btContratoClick(Sender: TObject);
begin
   If Objeto.ValidaAccesoEsp('TCRLINGPP_AGRLI',True,True) then
   Begin
...
Procedure TWCrLinGPPerd.InsertaDato;
Var
   STPInserta   : TStoredProc;
   vlError, sSQL, vlCve : String;
   qyQuery : TQuery;
Begin
   sSQL:='SELECT ID_SOLICITUD, ID_CONTRATO FROM CR_GA_GAR_CTO_PP '+
   ...
   If (vlCve = '' ) Then Begin
      ...
      Try
         With STPInserta Do Begin
            ...
            STPInserta.StoredProcName:='PKGCRDOCUMENTACION.STP_INSERTA_GARCTO_GPP';
            Params.Clear;
...
procedure TWCrLinGPPerd.InterForma1DespuesShow(Sender: TObject);
begin
   Objeto.ID_SOLICITUD.GetFromControl;
...
function TCrLinGPP.BuscaContrato: string;
var
  strResultado : string;
  T            : TInterFindit;
begin
...
procedure TWCrLinGPPerd.btEliminarClick(Sender: TObject);
Var
   STPInserta   : TStoredProc;
   vlError, sSQL : String;
   qyQuery : TQuery;
Begin
   If Objeto.ValidaAccesoEsp('TCRLINGPP_ELILI',True,True) then
   Begin
     Objeto.ID_SOLICITUD.GetFromControl;
     Objeto.ID_CONTRATO.GetFromControl;
...
Procedure TWCrLinGPPerd.MuestraDatos;
var
   vlQry : TQuery;
   sSQL :String;
begin
...
Function TWCrLinGPPerd.CadenaSql : String;
var
   vlsql : String;
Begin
   If Objeto.ID_SOLICITUD.AsInteger <> 0 Then Begin
      vlSql:='SELECT CR_GA_GAR_CTO_PP.ID_CONTRATO, '+
             ...
             '       CR_GA_GAR_CTO_PP.F_ALTA, '+
             '       CR_GA_GAR_CTO_PP.CVE_USU_ALTA '+
             '  FROM CR_GA_GAR_CTO_PP, CR_CONTRATO, CR_PRODUCTO '+
             ' WHERE CR_GA_GAR_CTO_PP.ID_CONTRATO = CR_CONTRATO.ID_CONTRATO '+
             '   AND CR_CONTRATO.CVE_PRODUCTO_CRE = CR_PRODUCTO.CVE_PRODUCTO_CRE '+
             '   AND CR_GA_GAR_CTO_PP.ID_SOLICITUD ='+ Objeto.ID_SOLICITUD.AsString +
...
procedure TWCrLinGPPerd.InterForma1Buscar(Sender: TObject);
begin
  Self.Objeto.Busca;
end;

procedure TWCrLinGPPerd.sgcDATAMoveRow(Sender: TObject; FromIndex,
  ToIndex: Integer);
begin
...
procedure TWCrLinGPPerd.btnBitCambiosClick(Sender: TObject);
begin
   If Objeto.ValidaAccesoEsp('TCRLINGPP_BITCA',True,True) then
   Begin
     if (Objeto.ID_SOLICITUD.AsString <> '') then
     Begin
         {$WARNINGS OFF}
         Objeto.BCLiGPP := TCRBCLiGPP.Create(Self);
         {$WARNINGS ON}
         try
            Objeto.BCLiGPP.Apli:=Objeto.Apli;
            Objeto.BCLiGPP.ParamCre:=Objeto.ParamCre;
            Objeto.BCLiGPP.vgIdSolicitud:= Objeto.ID_SOLICITUD.AsString;
            Objeto.BCLiGPP.Catalogo;
         finally
               if Objeto.BCLiGPP <> nil then
                  Objeto.BCLiGPP.Free;
               //end if
         end;
     end

// Sistema         : Clase de TMPrAdeuF
// Fecha de Inicio : 02/08/2004
// Función forma   : Clase de TMPrAdeuF
...
constructor TMPrAdeuF.Create( AOwner : TComponent );
begin Inherited;
      ...
      //
      UseQuery := False;
      HelpFile := 'InTMPrAdeuF.Hlp';
      ShowMenuReporte:=True;

      VGID_CREDITO := 0;
end;

Destructor TMPrAdeuF.Destroy;
begin
   if Credito <> nil then
      Credito.Free;
   if Empresa <> nil then
...
function TMPrAdeuF.ShowWindow(FormaTipo:TFormaTipo):Integer;
var W : TwMPrAdeuF;
begin
   W:=TwMPrAdeuF.Create(Self);
   try
      W.Objeto:= Self;
...
Function TMPrAdeuF.InternalBusca:Boolean;
begin
  InternalBusca := False;
end;
...
function TwMPrAdeuF.funObtFechaMaxima: TDate;
var vlSQL : String;
    Qry   : TQuery;
begin
...
procedure TwMPrAdeuF.AsignaEmpresaSuc;
begin
...
Procedure TwMPrAdeuF.DeterminaFecha(PPBValCre: String);
var Qry   : Tquery;
    VLSQL : String;
begin
...
procedure TwMPrAdeuF.FormShow(Sender: TObject);
begin
...
procedure TwMPrAdeuF.FormDestroy(Sender: TObject);
begin
...
procedure TwMPrAdeuF.FormClose(Sender: TObject; var Action: TCloseAction);
begin Action := caFree;
end;

procedure TwMPrAdeuF.InterForma1DespuesNuevo(Sender: TObject);
begin //<<Control>>.SetFocus;
end;

procedure TwMPrAdeuF.InterForma1DespuesModificar(Sender: TObject);
begin //<<Control>>.SetFocus;
end;

procedure TwMPrAdeuF.InterForma1BtnPreviewClick(Sender: TObject);
begin
   if Objeto.ValidaAccesoEsp('TMPRADEUF_PREVI',True,True) then
   begin
      ImprimeReporte(True);    
   end;
end;

Procedure TwMPrAdeuF.ImprimeReporte(bPreview:Boolean);
var vFMax : TDate;
    Qry   : TQuery;
    VLSQL : String;
    BNivela : String;
...
End;
procedure TwMPrAdeuF.InterForma1BtnImprimirClick(Sender: TObject);
begin
   if Objeto.ValidaAccesoEsp('TMPRADEUF_IMPRI',True,True) then
   begin
      ImprimeReporte(False);
   end;
end;

procedure TwMPrAdeuF.edID_CESIONChange(Sender: TObject);
begin
...
procedure TwMPrAdeuF.btID_CESIONClick(Sender: TObject);
var vlMsg : String;
begin
...
procedure TwMPrAdeuF.edID_CESIONExit(Sender: TObject);
begin
    InterForma1.ValidaExit(edID_CESION,True,'',True);
end;

procedure TwMPrAdeuF.chbxCESIONClick(Sender: TObject);
begin
...
procedure TwMPrAdeuF.ilCesionCapture(Sender: TObject;
  var Show: Boolean);
begin
   Show:=True;
end;

procedure TwMPrAdeuF.ilCesionEjecuta(Sender: TObject);
Var vlMsg: String;
begin
...
procedure TwMPrAdeuF.cbxFECHASClick(Sender: TObject);
begin
...
procedure TwMPrAdeuF.EnableCheckBox(var CheckBox : TCheckBox);
begin
...
procedure TwMPrAdeuF.edFIniChange(Sender: TObject);
begin
...
procedure TwMPrAdeuF.edFFinChange(Sender: TObject);
begin
...
procedure TwMPrAdeuF.rgFechaClick(Sender: TObject);
begin
     DeterminaFecha('V');
end;

procedure TwMPrAdeuF.bbEmpresaClick(Sender: TObject);
begin
   Objeto.Empresa.ShowAll := True;
   Objeto.Empresa.Busca;
end;

procedure TwMPrAdeuF.BBSucursalClick(Sender: TObject);
begin
...
procedure TwMPrAdeuF.ilEmpresaEjecuta(Sender: TObject);
begin
   Objeto.Empresa.FindKey([ilEmpresa.Buffer])
end;

procedure TwMPrAdeuF.ILSucursalEjecuta(Sender: TObject);
begin
...
procedure TwMPrAdeuF.CBSucursalClick(Sender: TObject);
begin

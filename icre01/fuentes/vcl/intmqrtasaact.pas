...
1. Se anexa procedimiento para calcular Tasa de Interes al día para el cualculo de Financiamientos Adicionales
...
Begin
      QrTasaAct   := TQrTasaAct.Create(Nil);
      ...
      Try
        ...
        QrTasaAct.QRInterEncabezado1.Titulo1:='Reporte Pronóstico de Adeudo ' + sCesion;
        ...
        QrTasaAct.QRLabel89.Caption := 'Adeudo Vigente al : ' + DateToStr(pAPli.DameFechaEmpresa);
        QrTasaAct.QRLabel26.Caption := 'Programado al : ' + Trim(sFechaIni);
        QrTasaAct.QRLabel64.Caption := 'Pago Total al : ' + Trim(sFechaIni);
...
Function TQrTasaAct.LlenaEncabezado(sCesion,sFechaIni, sFechaFin:String; pAPli : TInterApli) : String;
Var
    sSQL1:String;
    sSQL2:String;
    sSQL21:String;
...
//HERJA 26.08.2010
Function TQrTasaAct.CalculoTasa(vlidcredito, vlperiodo:integer):single;
...
Procedure TQrTasaAct.Intereses;
var stpDigVer : TStoredProc;
    Qry       : Tquery;
    vlStrSql  : String;
    vlFint    : TDate;
    vlIEntro  : Integer;                //--- ASOR --- MARZO 2016 ---
begin
 VGInteres := 0;
 VGInteresIva := 0;
 VGInteresTrans := 0;
 ...
 vlStrSql := 'SELECT ID_CREDITO,  '   +coCRLF +
             ...
             '               NVL(CC.B_INT_DIASIG,''F'') B_INT_DIASIG, ' +coCRLF +
             '               PKGCRCOMUN.STPOBTFVENCIMIENTO( CI.ID_CREDITO, CI.NUM_PERIODO, CI.F_VENCIMIENTO ) F_VENCTO ' +coCRLF +
             '        FROM   CR_INTERES  CI, CR_CREDITO CC '   +coCRLF +
             '        WHERE  CI.ID_CREDITO = ' + qyEncabezadoID_CREDITO.AsString +coCRLF +
             '        AND    CI.ID_CREDITO = CC.ID_CREDITO ' +coCRLF +
             '       ) ' +coCRLF +
             'WHERE  '   + SQLFecha( StrToDate(VGFechaIni) )  + ' BETWEEN CASE WHEN B_INT_DIASIG = ''V'' THEN F_INICIO ELSE  F_INICIO + 1 END AND F_VENCTO' + coCRLF;
    ...
    begin
       ...
       If Assigned(stpDigVer) Then
        With stpDigVer Do
         ...
         begin
            //--- ASOR --- ABRIL 2016 ---
            If Apli.DameFechaEmpresa = Qry.FieldByName('F_VENCIMIENTO').AsDateTime then
              begin
                ParamByName('PEFREFERENCIA').AsDateTime := (Apli.DameFechaEmpresa - 1);
                ParamByName('PEFCALCULO').AsDateTime    := (Apli.DameFechaEmpresa - 1);
                vlIEntro:= 1;
              end
            else
              begin
         ...
         begin
           ...
           VGTasaTrans       := FormatFloat('##.000000', ParamByName('PSTASA').AsFloat );
         end;
        Finally
         Close;
         Params.Clear;
       ...
       begin
           ...
           If Assigned(stpDigVer) Then
            With stpDigVer Do
             ...
             if Qry.FieldByName('TIPO_PERIODO').AsString  = 'A' then begin
               //--- ASOR --- MARZO 2016 ---
               if (Qry.FieldByName('F_VENCIMIENTO').AsDateTime = StrToDateTime(VGFechaIni)) then
                 begin
                     vlIEntro:= 1;
                     ParamByName('PEFCALCULO').AsDateTime := (StrToDateTime(VGFechaIni) - 1);
                 end
               else
             ...
             if Qry.FieldByName('TIPO_PERIODO').AsString  = 'A' then begin
               ...
               VGTasaProyTrans   := FormatFloat('##.000000', ParamByName('PSTASA').AsFloat );
             end;
            Finally
             Close;
             Params.Clear;
...
Function TQrTasaAct.ReferenciaDigVer(strReferencia : String; Apli : TInterApli) : String;
var stpDigVer : TStoredProc;
begin
 Result := '';
 stpDigVer := TStoredProc.Create(Nil);
...
procedure TQrTasaAct.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
    Group1.Enabled:= True;
    Group3.Enabled:= True;
...
Procedure TQrTasaAct.HabilitaGrupos;
Begin
...
procedure TQrTasaAct.Group1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
   Self.HabilitaGrupos;
   dSumaINTVENCIDO:=0;
...
procedure TQrTasaAct.Group3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Self.HabilitaGrupos;
  dSumaCAPVENCIDO:=0;
...
procedure TQrTasaAct.Group7BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Self.HabilitaGrupos;
end;

Function TQrTasaAct.ObtieneTasa(dCredito: Double;dPeriodo : Integer; sAccesorio: String):Double;
Var
...
Function TQrTasaAct.ObtieneTasaMoras(dCredito: Double;dPeriodo : Integer; sAccesorio: String):Double;
Var
   STP_TASA : TStoredProc;
   vlError : String;
Begin
...
Function TQrTasaAct.LlenaConsulta(sCesion,sFechaIni, sFechaFin,
                                  sEmpresa, sSucursal:String;
...
procedure TQrTasaAct.QRFoot1BeforePrint(Sender: TQRCustomBand;  var PrintBand: Boolean);
begin
...
procedure TQrTasaAct.QRFoot2BeforePrint(Sender: TQRCustomBand;  var PrintBand: Boolean);
begin
...
procedure TQrTasaAct.Group5BeforePrint(Sender: TQRCustomBand;   var PrintBand: Boolean);
begin
  Self.HabilitaGrupos;
  dSumaREFINAN01:= 0;
  dSumaREFINAN02:= 0;
...
procedure TQrTasaAct.QRFoot3BeforePrint(Sender: TQRCustomBand;  var PrintBand: Boolean);
begin
...
procedure TQrTasaAct.QRGroup7BeforePrint(Sender: TQRCustomBand;  var PrintBand: Boolean);
...
procedure TQrTasaAct.Group9BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Self.HabilitaGrupos;
...
procedure TQrTasaAct.QRFoot4BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TQrTasaAct.QRFoot5BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TQrTasaAct.Group10BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Self.HabilitaGrupos;
  dSumaCOMVENCIDO:=0;
...
procedure TQrTasaAct.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin

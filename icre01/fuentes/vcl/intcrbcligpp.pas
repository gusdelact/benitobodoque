...
constructor TCRBCLiGPP.Create( AOwner : TComponent );
begin Inherited;
    StpName  := ' ';
    UseQuery := False;
    HelpFile := 'InTCRBCLiGPP.Hlp';
    ShowMenuReporte:=True;
end;

Destructor TCRBCLiGPP.Destroy;
begin
   //end if;
...
function TCRBCLiGPP.ShowWindow(FormaTipo:TFormaTipo):Integer;
var W : TWCRBCLiGPP;
begin
   W:=TWCRBCLiGPP.Create(Self);
   try
      W.Objeto:= Self;
...
Function TCRBCLiGPP.InternalBusca:Boolean;
Var T:TInterFindit;
...
function TCRBCLiGPP.Reporte:Boolean; 
...
procedure TWCRBCLiGPP.FormShow(Sender: TObject);
begin
      //Objeto
...
procedure TWCRBCLiGPP.FormDestroy(Sender: TObject);
begin
   //Objeto
end;

procedure TWCRBCLiGPP.FormClose(Sender: TObject; var Action: TCloseAction);
begin Action := caFree;
end;

procedure TWCRBCLiGPP.InterForma1DespuesNuevo(Sender: TObject);
begin //<<Control>>.SetFocus;
end;

procedure TWCRBCLiGPP.InterForma1DespuesModificar(Sender: TObject);
begin //<<Control>>.SetFocus;
end;
...
procedure TWCRBCLiGPP.chkFechaAltaClick(Sender: TObject);
begin
...
procedure TWCRBCLiGPP.bGeneraClick(Sender: TObject);
begin
   If Objeto.ValidaAccesoEsp('TCRBCLIGPP_CONSU',True,True) then
   Begin
       //Validaciones
...
procedure TWCRBCLiGPP.ConsultaRegistros;
var
    Q               : TQuery;
    VGSQLGenCons    : String;
begin
    ...
    VGSQLGenCons := ' select ' + #13 +
                    ...
                    '                           where  h.origen_datos = ''CR_GA_GAR_CTO_PP'' ' + #13 +

...
constructor TCRBitCGPP.Create( AOwner : TComponent );
begin Inherited;
    StpName  := ' ';
    UseQuery := False;
    HelpFile := 'InTCRBitCGPP.Hlp';
    ShowMenuReporte:=True;
end;

Destructor TCRBitCGPP.Destroy;
begin
   //end if;
...
function TCRBitCGPP.ShowWindow(FormaTipo:TFormaTipo):Integer;
var W : TWCRBitCGPP;
begin
   W:=TWCRBitCGPP.Create(Self);
   try
      W.Objeto:= Self;
...
Function TCRBitCGPP.InternalBusca:Boolean;
Var T:TInterFindit;
...
function TCRBitCGPP.Reporte:Boolean; 
...
procedure TWCRBitCGPP.FormShow(Sender: TObject);
begin
      //Objeto
...
procedure TWCRBitCGPP.FormDestroy(Sender: TObject);
begin
   //Objeto
end;

procedure TWCRBitCGPP.FormClose(Sender: TObject; var Action: TCloseAction);
begin Action := caFree;
end;

procedure TWCRBitCGPP.InterForma1DespuesNuevo(Sender: TObject);
begin //<<Control>>.SetFocus;
end;

procedure TWCRBitCGPP.InterForma1DespuesModificar(Sender: TObject);
begin //<<Control>>.SetFocus;
end;
...
procedure TWCRBitCGPP.chkFechaAltaClick(Sender: TObject);
begin
...
procedure TWCRBitCGPP.bGeneraClick(Sender: TObject);
begin
   If Objeto.ValidaAccesoEsp('TCRBITCGPP_CONSU',True,True) then
   Begin
       //Validaciones
...
procedure TWCRBitCGPP.ConsultaRegistros;
var
    Q               : TQuery;
    VGSQLGenCons    : String;
begin
    ...
    VGSQLGenCons := ' select ' + #13 +
                    ...
                    ' --     case when NVL(ID_SOLICITUD, 0) <> lag(NVL(ID_SOLICITUD, 0)) over (partition by ID_SOLICITUD order by fh_registro) then ''SI'' else ''NO'' end cambio_ID_SOLICITUD, ' + #13 +
                    ...
                    '      case when NVL(CVE_BCO_GTIA, '' '') <> lag(NVL(CVE_BCO_GTIA, '' '')) over (partition by ID_SOLICITUD order by fh_registro) then ''SI'' else ''NO'' end cambio_CVE_BCO_GTIA, ' + #13 +
                    '      CAST(regs.TX_DESC_PORTAF as VARCHAR2(250)) as TX_DESC_PORTAF, ' + #13 +
                    '      case when NVL(TX_DESC_PORTAF, '' '') <> lag(NVL(TX_DESC_PORTAF, '' '')) over (partition by ID_SOLICITUD order by fh_registro) then ''SI'' else ''NO'' end cambio_TX_DESC_PORTAF, ' + #13 +
                    '      CAST(regs.CVE_DET_MONTO as VARCHAR2(2)) as CVE_DET_MONTO, ' + #13 +
                    '      case when NVL(CVE_DET_MONTO, '' '') <> lag(NVL(CVE_DET_MONTO, '' '')) over (partition by ID_SOLICITUD order by fh_registro) then ''SI'' else ''NO'' end cambio_CVE_DET_MONTO, ' + #13 +
                    '      CAST(regs.SIT_REG_GARANTIA as VARCHAR2(2)) as SIT_REG_GARANTIA, ' + #13 +
                    '      case when NVL(SIT_REG_GARANTIA, '' '') <> lag(NVL(SIT_REG_GARANTIA, '' '')) over (partition by ID_SOLICITUD order by fh_registro) then ''SI'' else ''NO'' end cambio_SIT_REG_GARANTIA, ' + #13 +
                    '      CAST(regs.MONTO_GARANTIA as NUMBER(16,2)) as MONTO_GARANTIA, ' + #13 +
                    '      case when NVL(MONTO_GARANTIA, 0) <> lag(NVL(MONTO_GARANTIA, 0)) over (partition by ID_SOLICITUD order by fh_registro) then ''SI'' else ''NO'' end cambio_MONTO_GARANTIA, ' + #13 +
                    '      CAST(regs.PCT_GARANTIA as NUMBER(8,4)) as PCT_GARANTIA, ' + #13 +
                    '      case when NVL(PCT_GARANTIA, 0) <> lag(NVL(PCT_GARANTIA, 0)) over (partition by ID_SOLICITUD order by fh_registro) then ''SI'' else ''NO'' end cambio_PCT_GARANTIA, ' + #13 +
                    '      to_date(regs.F_ALTA, ''DD/MM/YYYY HH12:MI:SS AM'') as F_ALTA, ' + #13 +
                    '      case when NVL(F_ALTA, TO_DATE(''01/01/1981'',''DD/MM/YYYY'')) <> lag(NVL(F_ALTA, TO_DATE(''01/01/1981'',''DD/MM/YYYY''))) over (partition by ID_SOLICITUD order by fh_registro) then ''SI'' else ''NO'' end cambio_F_ALTA, ' + #13 +
                    '      CAST(regs.CVE_USU_ALTA as VARCHAR2(8)) as CVE_USU_ALTA, ' + #13 +
                    '      case when NVL(CVE_USU_ALTA, '' '') <> lag(NVL(CVE_USU_ALTA, '' '')) over (partition by ID_SOLICITUD order by fh_registro) then ''SI'' else ''NO'' end cambio_CVE_USU_ALTA, ' + #13 +
                    '      CAST(regs.CVE_USU_MODIFICA as VARCHAR2(8)) as CVE_USU_MODIFICA, ' + #13 +
                    '      case when NVL(CVE_USU_MODIFICA, '' '') <> lag(NVL(CVE_USU_MODIFICA, '' '')) over (partition by ID_SOLICITUD order by fh_registro) then ''SI'' else ''NO'' end cambio_CVE_USU_MODIFICA ' + #13 +
                    ' from   (select   * ' + #13 +
                    '         from     (select case ' + #13 +
                    '                            when lag (b_ad) over (partition by id_solicitud order by id_audit) is null ' + #13 +
                    '                          and   id_solicitud = lead (id_solicitud) over (partition by id_solicitud order by id_audit) ' + #13 +
                    '                              then 1 ' + #13 +
                    '                            when lead (b_ad) over (partition by id_solicitud order by id_audit) is null ' + #13 +
                    '                              then 1 ' + #13 +
                    '                            else 0 ' + #13 +
                    '                          end l, ' + #13 +
                    '                          his.* ' + #13 +
                    ...
                    '                           where  h.origen_datos = ''CR_GA_PP_REGISTRO'' ' + #13 +

...
type
  TQrInCoCob3 = class(TQuickRep)
    ...
    procedure SummaryBand1BeforePrint(Sender: TQRCustomBand;
      var PrintBand: Boolean);
    procedure QRGroupDispBeforePrint(Sender: TQRCustomBand;
      var PrintBand: Boolean);
  private
     bQrResumen: Boolean;
  public
...
Procedure RepInfCobrConta3(  sF_Inicio,sF_Final,sProducto,sMoneda,
                            sAcreditado, sCatMinimo, sCredito,
...
Begin
   QrInCoCob3:=TQrInCoCob3.Create(Nil);
   Preview := TIntQRPreview.CreatePreview(Application, QrInCoCob3);
   ...
   Try
      bGenArch:= False;
      sArchivo:= sNbrArch;
      QrInCoCob3.bQrResumen := bResumen;
      bDetDD := bDetalleDD; //EASA4011 29122004
      QrInCoCob3.QRInterEncabezado1.Apli:=pApli;
      QrInCoCob3.QRInterEncabezado1.IdEmpresa:= pAPli.IdEmpresa;
      QrInCoCob3.QRInterEncabezado1.NomReporte:='IntMQrInCoCob3';
      QrInCoCob3.QRInterEncabezado1.Titulo1:='Informe Contable de Cobranza Área de Contabilidad';

      If (Trim(sSitMov) = 'AC') Then
          QrInCoCob3.QRInterEncabezado1.Titulo2:=QrInCoCob3.QRInterEncabezado1.Titulo2 + 'Movtos ACTIVOS ';

      If (Trim(sSitMov) = 'CA') Then
          QrInCoCob3.QRInterEncabezado1.Titulo2:=QrInCoCob3.QRInterEncabezado1.Titulo2 + 'Movtos CANCELADOS ';

      If Trim(sF_Inicio)<>'' Then
          QrInCoCob3.QRInterEncabezado1.Titulo2:=QrInCoCob3.QRInterEncabezado1.Titulo2 + ' del '+
                                      sF_Inicio + ' al ' + sF_Final;
      if bResumen then Begin
         QrInCoCob3.QRGroupDisp.Enabled := False;
         QrInCoCob3.qrbCONCEPTOS.Enabled := False;
         QrInCoCob3.QRFooterCesion.Enabled := False;
         QrInCoCob3.QRInterEncabezado1.Titulo1:= 'Resumen del ' + QrInCoCob3.QRInterEncabezado1.Titulo1;
      end
      else Begin
         if not bDetalleDD then
            QrInCoCob3.QRInterEncabezado1.Titulo2:= QrInCoCob3.QRInterEncabezado1.Titulo2 + ' ( No Incluye Det de Doctos. )'
         else
            QrInCoCob3.QRInterEncabezado1.Titulo2:= QrInCoCob3.QRInterEncabezado1.Titulo2 + ' ( Incluye Det de Doctos. )';
      end;

      if bPagoFIRA = 0 then
          QrInCoCob3.QRInterEncabezado1.Titulo2:=QrInCoCob3.QRInterEncabezado1.Titulo2 + ' (SOLO BINTER) '
      else if bPagoFIRA = 1 then
          QrInCoCob3.QRInterEncabezado1.Titulo2:=QrInCoCob3.QRInterEncabezado1.Titulo2 + ' (SOLO FIRA) '
      else
          QrInCoCob3.QRInterEncabezado1.Titulo2:=QrInCoCob3.QRInterEncabezado1.Titulo2 + ' (INDISTINTO) ';
      //end if


      QrInCoCob3.qyInfCobConta.DatabaseName := pApli.DataBaseName;
      QrInCoCob3.qyInfCobConta.SessionName := pApli.SessionName;

      QrInCoCob3.QRLbSituacion.Visible := False; //ales
      QrInCoCob3.QRLbSituacion.Enabled := False; //ales

      QrInCoCob3.qyInfCobConta.Active:=False;
      QrInCoCob3.qyInfCobConta.SQL.Text:= QrInCoCob3.FormaQuery(sF_Inicio,sF_Final,sProducto,sMoneda,
                      sAcreditado, sCatMinimo, sCredito,sSitMov, sOrden,
                      sEmpresa, sSucursal, sGpoProd,bIncTipoSec, bPagoFIRA);
      QrInCoCob3.qyInfCobConta.SQL.SaveToFile('C:\COBCONT.TXT');
      QrInCoCob3.qyInfCobConta.Active:=True;
      ...
      If Trim(sArchivo)<>'' Then Begin
         ...
         Rewrite(F);
         Writeln(F, QrInCoCob3.QRInterEncabezado1.Titulo1 + #09 );
         Writeln(F, QrInCoCob3.QRInterEncabezado1.Titulo2 + #09 );
         Writeln(F, #09 );
         ...
         Writeln(F, #09#09#09#09#09#09#09#09#09#09#09#09#09#09#09#09#09#09 +
             'Exigible' +#09 + // 1314 01
             'No Exigible'); // 1314 02
         Writeln(F, '');
      End;
      ...
      begin
         ...
         sDigDocFileName := 'INF_COB_CONT_'  + FormatDateTime( 'DD_MM_YYYY', StrToDate(sF_Inicio)) + '_' +
                            ...
                            IntToStr( bPagoFIRA );
         QrInCoCob3.InterDigDoc1.RootDir  := obtValorGpoCve( 'CONFRH', 'ROOTDIR',   pApli );
         QrInCoCob3.InterDigDoc1.HostName := obtValorGpoCve( 'CONFRH', 'FTPSERVER', pApli );
         QrInCoCob3.InterDigDoc1.UserName := obtValorGpoCve( 'CONFRH', 'USERNAME',  pApli );
         QrInCoCob3.InterDigDoc1.Password := HexToStr( obtValorGpoCve( 'CONFRH', 'PASSWORD',  pApli ) );
         QrInCoCob3.InterDigDoc1.SendTo   := ObtSendTo( sF_Inicio );
         QrInCoCob3.InterDigDoc1.FileName := sArchivoDig; // sDigDocFileName; //<LOLS 17FEB2006. DIGITALIZACION DE REPORTES>
         QrInCoCob3.PrinterSettings.PrinterIndex := QrInCoCob3.InterDigDoc1.SetPrinter;
         QrInCoCob3.Print;
...
//       Para hacer el llenado de la tabla en la redigitalización.
         bDigitalizaCorrecta := ( QrInCoCob3.InterDigDoc1.Compress and QrInCoCob3.InterDigDoc1.SendFile);

//         QrInCoCob3.InterDigDoc1.Compress;
//         QrInCoCob3.InterDigDoc1.SendFile;
         QrInCoCob3.PrinterSettings.PrinterIndex := QrInCoCob3.InterDigDoc1.RestorePrinter;
      end;
        //If pPreview Then
        //    QrInCoCob3.Preview
        //Else
...
Function TQrInCoCob3.FormaQuery( sF_Inicio,sF_Final,sProducto,sMoneda,
                                sAcreditado, sCatMinimo, sCredito,
                                sSitMov, sOrden, sEmpresa, sSucursal, sGpoProd:String;
                                bIncTipoSec :Boolean;
...
procedure TQrInCoCob3.qyInfCobContaCalcFields(DataSet: TDataSet);
Var
    sFecha:String;
begin
...
procedure TQrInCoCob3.qrbCONCEPTOSBeforePrint(Sender: TQRCustomBand;  var PrintBand: Boolean);
Var
   sRegistro: String;
   iImporte1: Double;
begin
...
procedure TQrInCoCob3.QRFooterCesionBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TQrInCoCob3.QRFooterProductoBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
   If sArchivo <>'' Then Begin
...
procedure TQrInCoCob3.QRFooterMonedaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
   If sArchivo <>'' Then Begin
...
procedure TQrInCoCob3.SummaryBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TQrInCoCob3.QRGroupDispBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
Var
   sRegistro: String;
   iImporte1: Double;
begin
// RUCJ4248
// Se agrega con la intencion que para los creditos de FACTORAJE
// IMPRIMA sus encabezados en el reporte que se exporta a excel

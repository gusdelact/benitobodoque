...
var
  QrInCoCob: TQrInCoCob;
  bGenArch, bDetDD: Boolean;
  sArchivo:String;
  F: TextFile;

Procedure RepInfCobrConta(  sF_Inicio,sF_Final,sProducto,sMoneda,
                            sAcreditado, sCatMinimo, sCredito,
...
Begin
   QrInCoCob:=TQrInCoCob.Create(Nil);
   Preview := TIntQRPreview.CreatePreview(Application, QrInCoCob);
   Try
      bGenArch:= False;
      sArchivo:= sNbrArch;
      QrInCoCob.bQrResumen := bResumen;
      bDetDD := bDetalleDD; //EASA4011 29122004
      QrInCoCob.QRInterEncabezado1.Apli:=pApli;
      QrInCoCob.QRInterEncabezado1.IdEmpresa:= pAPli.IdEmpresa;
      QrInCoCob.QRInterEncabezado1.NomReporte:='IntMQrInCoCob';
      QrInCoCob.QRInterEncabezado1.Titulo1:='Informe Contable de Cobranza Área de Contabilidad';

      If (Trim(sSitMov) = 'AC') Then
          QrInCoCob.QRInterEncabezado1.Titulo2:=QrInCoCob.QRInterEncabezado1.Titulo2 + 'Movtos ACTIVOS ';

      If (Trim(sSitMov) = 'CA') Then
          QrInCoCob.QRInterEncabezado1.Titulo2:=QrInCoCob.QRInterEncabezado1.Titulo2 + 'Movtos CANCELADOS ';

      If Trim(sF_Inicio)<>'' Then
          QrInCoCob.QRInterEncabezado1.Titulo2:=QrInCoCob.QRInterEncabezado1.Titulo2 + ' del '+
                                      sF_Inicio + ' al ' + sF_Final;
      if bResumen then Begin
         QrInCoCob.QRGroupDisp.Enabled := False;
         QrInCoCob.qrbCONCEPTOS.Enabled := False;
         QrInCoCob.QRFooterCesion.Enabled := False;
         QrInCoCob.QRInterEncabezado1.Titulo1:= 'Resumen del ' + QrInCoCob.QRInterEncabezado1.Titulo1;
      end
      else Begin
         if not bDetalleDD then
            QrInCoCob.QRInterEncabezado1.Titulo2:= QrInCoCob.QRInterEncabezado1.Titulo2 + ' ( No Incluye Det de Doctos. )'
         else
            QrInCoCob.QRInterEncabezado1.Titulo2:= QrInCoCob.QRInterEncabezado1.Titulo2 + ' ( Incluye Det de Doctos. )';
      end;

      if bPagoFIRA = 0 then
          QrInCoCob.QRInterEncabezado1.Titulo2:=QrInCoCob.QRInterEncabezado1.Titulo2 + ' (SOLO BINTER) '
      else if bPagoFIRA = 1 then
          QrInCoCob.QRInterEncabezado1.Titulo2:=QrInCoCob.QRInterEncabezado1.Titulo2 + ' (SOLO FIRA) '
      else
          QrInCoCob.QRInterEncabezado1.Titulo2:=QrInCoCob.QRInterEncabezado1.Titulo2 + ' (INDISTINTO) ';
      //end if


      QrInCoCob.qyInfCobConta.DatabaseName := pApli.DataBaseName;
      QrInCoCob.qyInfCobConta.SessionName := pApli.SessionName;

      QrInCoCob.QRLbSituacion.Visible := False; //ales
      QrInCoCob.QRLbSituacion.Enabled := False; //ales

      QrInCoCob.qyInfCobConta.Active:=False;
      QrInCoCob.qyInfCobConta.SQL.Text:= QrInCoCob.FormaQuery(sF_Inicio,sF_Final,sProducto,sMoneda,
                      sAcreditado, sCatMinimo, sCredito,sSitMov, sOrden,
                      sEmpresa, sSucursal, sGpoProd,bIncTipoSec, bPagoFIRA);
      QrInCoCob.qyInfCobConta.SQL.SaveToFile('C:\COBCONT.TXT');
      QrInCoCob.qyInfCobConta.Active:=True;

      If Trim(sArchivo)<>'' Then Begin
         AssignFile(F, sArchivo);
         Rewrite(F);
         Writeln(F, QrInCoCob.QRInterEncabezado1.Titulo1 + #09 );
         Writeln(F, QrInCoCob.QRInterEncabezado1.Titulo2 + #09 );
         Writeln(F, #09 );
      ...
      begin
         ...
         sDigDocFileName := 'INF_COB_CONT_'  + FormatDateTime( 'DD_MM_YYYY', StrToDate(sF_Inicio)) + '_' +
                            ...
                            IntToStr( bPagoFIRA );
         QrInCoCob.InterDigDoc1.RootDir  := obtValorGpoCve( 'CONFRH', 'ROOTDIR',   pApli );
         QrInCoCob.InterDigDoc1.HostName := obtValorGpoCve( 'CONFRH', 'FTPSERVER', pApli );
         QrInCoCob.InterDigDoc1.UserName := obtValorGpoCve( 'CONFRH', 'USERNAME',  pApli );
         QrInCoCob.InterDigDoc1.Password := HexToStr( obtValorGpoCve( 'CONFRH', 'PASSWORD',  pApli ) );
         QrInCoCob.InterDigDoc1.SendTo   := ObtSendTo( sF_Inicio );
         QrInCoCob.InterDigDoc1.FileName := sArchivoDig; // sDigDocFileName; //<LOLS 17FEB2006. DIGITALIZACION DE REPORTES>
         QrInCoCob.PrinterSettings.PrinterIndex := QrInCoCob.InterDigDoc1.SetPrinter;
         QrInCoCob.Print;
...
//       Para hacer el llenado de la tabla en la redigitalización.
         bDigitalizaCorrecta := ( QrInCoCob.InterDigDoc1.Compress and QrInCoCob.InterDigDoc1.SendFile);

//         QrInCoCob.InterDigDoc1.Compress;
//         QrInCoCob.InterDigDoc1.SendFile;
         QrInCoCob.PrinterSettings.PrinterIndex := QrInCoCob.InterDigDoc1.RestorePrinter;
      end;
        //If pPreview Then
        //    QrInCoCob.Preview
        //Else
...
Function TQrInCoCob.FormaQuery( sF_Inicio,sF_Final,sProducto,sMoneda,
                                sAcreditado, sCatMinimo, sCredito,
                                sSitMov, sOrden, sEmpresa, sSucursal, sGpoProd:String;
                                bIncTipoSec :Boolean;
...
procedure TQrInCoCob.qyInfCobContaCalcFields(DataSet: TDataSet);
Var
    sFecha:String;
begin
...
procedure TQrInCoCob.qrbCONCEPTOSBeforePrint(Sender: TQRCustomBand;  var PrintBand: Boolean);
Var
   sRegistro: String;
   iImporte1: Double;
begin
...
procedure TQrInCoCob.QRFooterCesionBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TQrInCoCob.QRFooterProductoBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
   If sArchivo <>'' Then Begin
...
procedure TQrInCoCob.QRFooterMonedaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
   If sArchivo <>'' Then Begin
...
procedure TQrInCoCob.SummaryBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TQrInCoCob.QRGroupDispBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
Var
   sRegistro: String;
   iImporte1: Double;

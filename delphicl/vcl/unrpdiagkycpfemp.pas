unit UnRpDiagKYCPFEmp;
//------------------------------------------------------------------------------
//  Diagnóstico de KYC para Personas Fisicas Con Actividad Empresarial
...
type
  TqrDiagKYCPFEmp = class(TQuickRep)
    qPersona: TQuery;
    PageHeaderBand1: TQRBand;
    qrieDiagKYCPFEmp: TQRInterEncabezado;
    qContrato: TQuery;
...
var
  qrDiagDiagKYCPFEmp : TqrDiagKYCPFEmp;
  procedure EXECUTE_RP_DIAGKYCPFEmp(pApli: TInterApli; pVerImprimir: String; pIdContrato : Integer);
...
procedure EXECUTE_RP_DIAGKYCPFEmp(pApli: TInterApli; pVerImprimir: String; pIdContrato : Integer);
var
  qrDiagKYCPFEmp: TqrDiagKYCPFEmp;
begin
  qrDiagKYCPFEmp := TqrDiagKYCPFEmp.Create(Nil);
  with qrDiagKYCPFEmp do
  try
    Apli                  := pApli;
    qrieDiagKYCPFEmp.Apli := Apli;
    ...
    begin
      qrieDiagKYCPFEmp.Titulo2   := 'Contrato ' + IntToStr(pIdContrato);
      qrieDiagKYCPFEmp.IdEmpresa := qContrato.FieldByName('ID_EMPRESA').AsInteger;
      qPersona.Open;
...
procedure TqrDiagKYCPFEmp.Diagnostica(pQuery       : TQuery;
                                  var pQRMemoTiene : TQRMemo;
                                  var pQRMemoFalta : TQRMemo);
var
  vlContador    : Integer;
...
procedure TqrDiagKYCPFEmp.DiagnosticaNum(pQuery       : TQuery;
                                     var pQRMemoTiene : TQRMemo;
                                     var pQRMemoFalta : TQRMemo);
var
  vlContador    : Integer;
...
procedure TqrDiagKYCPFEmp.qrchPerEmpBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TqrDiagKYCPFEmp.QRGroup1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qCtoOpera.Close;
  qCtoOpera.Open;
...
procedure TqrDiagKYCPFEmp.SummaryBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin

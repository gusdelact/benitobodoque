//------------------------------------------------------------------------------
// Función    : Tipo de Operaciones por Contrato
...
function TCtoOpAux.RangoLimite(pCveRangoOp : String) : Integer;
var
   Qry : TQuery;
begin
   try
...
procedure TCtoOpAux.AfterLocalizaRegistro(Sender : TObject);
begin
...
procedure TCtoOpAux.AfterLocalizaContrato(Sender : TObject);
var qProd: TQuery;
begin
...
constructor TCtoOpAux.Create( AOwner : TComponent );
begin
   Inherited;
...
destructor TCtoOpAux.Destroy;
begin
   inherited;
end;

procedure TCtoOpAux.DataBaseChange;
begin
   inherited;
...
function TCtoOpAux.ShowWindow(FormaTipo:TFormaTipo):Integer;
var
   W : TWCtoOpAux;
begin
   W := TWCtoOpAux.Create(Self);
   try
      W.Objeto                    := Self;
...
function TCtoOpAux.InternalBusca:Boolean;
var
   T : TInterFindit;
begin
   InternalBusca := False;
   ...
   try
      ...
      begin
         ...
         T.WhereString := T.WhereString + ' AND CTO_OPERACION_GRUPO.CVE_GRUPO_OPERACION = ' + QuotedStr('TIPOOP')
...
procedure TWCtoOpAux.sbContratoClick(Sender: TObject);
begin
...
procedure TWCtoOpAux.sbRangoClick(Sender: TObject);
begin
   Objeto.Registro.BuscaWhereString :=
    'CTO_OPERACION_GRUPO.CVE_GRUPO_OPERACION = ' + QuotedStr('TIPOOP')
...
procedure TWCtoOpAux.FormShow(Sender: TObject);
begin
   with Objeto do
   begin
...
procedure TWCtoOpAux.FormDestroy(Sender: TObject);
begin
   with Objeto do
   begin
...
procedure TWCtoOpAux.InterForma1AntesAceptar(Sender: TObject; IsNewData: Boolean; var Realizado: Boolean);
var
      ...
      try
         ...
         begin
            if GetSQL('select * '
               ...
               raise EInterFrame.Create('Ya existe el Tipo de Operacion para el Contrato');
         end;
         Realizado := True;
      except
         on e: EInterFrame do
...
procedure TWCtoOpAux.InterForma1DespuesNuevo(Sender: TObject);
begin
...
procedure TWCtoOpAux.InterForma1DespuesModificar(Sender: TObject);
begin
   vCveRangoOpAnt := trim(Objeto.Cve_Rango_Operacion.AsString);
   Objeto.AfterLocalizaRegistro(Sender);
end;

procedure TWCtoOpAux.InterForma1DespuesEliminar(Sender: TObject);
begin
   Objeto.ActivarEspecifique(False);
end;

procedure TWCtoOpAux.InterForma1DespuesAceptar(Sender: TObject);
begin
   Objeto.ActivarEspecifique(False);
end;

procedure TWCtoOpAux.InterForma1DespuesCancelar(Sender: TObject);
begin
...
procedure TWCtoOpAux.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   Action := caFree;
end;
end.
